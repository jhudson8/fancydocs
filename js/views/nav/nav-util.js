...
module.exports = {
  ...
  projectNavMenu: function(navItems, self, viewState, options) {
    ...
    return (
      <div className={"vertical ui menu " + (options.className || '')} key={options.key}>
        {children}
      </div>
    );
  },
...
function addMenuChildren (navItems, level, children, self, viewState, options) {
  ...
  _.each(navItems, function(navItem) {
    ...
    var hilighted = model && options.allowHilight && hilightModel && ((hilightModel.isEqual &&
      hilightModel.isEqual(model)) || hilightModel === model);
